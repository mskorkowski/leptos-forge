...
pub fn Switch<S: ToString>(
    ...
    let on_animationend = move |_: AnimationEvent| {
        if read.get_untracked() {
            use_swap_class(ref_button, ("animate-background-slide-left", "animate-background-slide-right", "bg-left"), "bg-right");
        }
        else {
            use_swap_class(ref_button, ("animate-background-slide-left", "animate-background-slide-right", "bg-right"), "bg-left");
        }
    };
